...
public class SyntaxException(Location location, string message) : RocketException(location, message)
{
    ...
    public static SyntaxException ExtendInputObjectTypeMissingAtLeastOne(Location location) => new(location, "Extend input object must specify at least one of directive or field list.");

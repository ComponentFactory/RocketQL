...
public class ValidationException(Location location, string message) : RocketException(location, message)
{
    ...
    public static ValidationException ExtendExistingEnumValueWithoutDirective(Location location, string enumValue, string enumTypeName) => new(location, $"Extend enum '{enumTypeName}' for existing enum value '{enumValue}' does not add at least one directive.");

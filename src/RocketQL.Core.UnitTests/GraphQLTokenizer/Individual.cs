...
public class Individual
{
    ...
    {
        var t = new Core.GraphQLTokenizer(text);
        Assert.Equal(GraphQLTokenKind.StartOfText, t.TokenKind);
        t.Next();
        Assert.Equal(GraphQLTokenKind.EndOfText, t.TokenKind);
        Assert.Equal(1, t.LineNumber);
    ...
    [Theory]
    [InlineData("!", GraphQLTokenKind.Exclamation)]
    [InlineData("$", GraphQLTokenKind.Dollar)]
    [InlineData("&", GraphQLTokenKind.Ampersand)]
    [InlineData("(", GraphQLTokenKind.LeftParenthesis)]
    [InlineData(")", GraphQLTokenKind.RightParenthesis)]
    [InlineData(":", GraphQLTokenKind.Colon)]
    [InlineData("=", GraphQLTokenKind.Equals)]
    [InlineData("@", GraphQLTokenKind.At)]
    [InlineData("[", GraphQLTokenKind.LeftSquareBracket)]
    [InlineData("]", GraphQLTokenKind.RightSquareBracket)]
    [InlineData("{", GraphQLTokenKind.LeftCurlyBracket)]
    [InlineData("}", GraphQLTokenKind.RightCurlyBracket)]
    [InlineData("|", GraphQLTokenKind.Vertical)]
    [InlineData("...", GraphQLTokenKind.Spread)]
    public void Punctuator(string text, GraphQLTokenKind token)
    {
        var t = new Core.GraphQLTokenizer(text);
        Assert.Equal(GraphQLTokenKind.StartOfText, t.TokenKind);
        t.Next();
    ...
    public void Name(string text)
    {
        ...
        Assert.Equal(GraphQLTokenKind.StartOfText, t.TokenKind);
        t.Next();
        Assert.Equal(GraphQLTokenKind.Name, t.TokenKind);
        Assert.Equal(text, t.TokenValue);
        Assert.Equal(1, t.ColumnNumber);
        t.Next();
        Assert.Equal(GraphQLTokenKind.EndOfText, t.TokenKind);
    ...
    [Theory]
    [InlineData("0", 0, GraphQLTokenKind.EndOfText)]
    [InlineData("9", 9, GraphQLTokenKind.EndOfText)]
    [InlineData("12345", 12345, GraphQLTokenKind.EndOfText)]
    [InlineData("9876543", 9876543, GraphQLTokenKind.EndOfText)]
    [InlineData("-0", -0, GraphQLTokenKind.EndOfText)]
    [InlineData("-9", -9, GraphQLTokenKind.EndOfText)]
    [InlineData("-12345", -12345, GraphQLTokenKind.EndOfText)]
    [InlineData("-9876543", -9876543, GraphQLTokenKind.EndOfText)]
    ...
    [InlineData("0#", 0, GraphQLTokenKind.EndOfText)]
    [InlineData("9\t", 9, GraphQLTokenKind.EndOfText)]
    [InlineData("12345\r\n", 12345, GraphQLTokenKind.EndOfText)]
    [InlineData("9876543,", 9876543, GraphQLTokenKind.EndOfText)]
    [InlineData("-0{", -0, GraphQLTokenKind.LeftCurlyBracket)]
    [InlineData("-0 {", -0, GraphQLTokenKind.LeftCurlyBracket)]
    [InlineData("-9876543\uFEFF", -9876543, GraphQLTokenKind.EndOfText)]
    public void IntValue(string text, int val, GraphQLTokenKind nextToken)
    {
        var t = new Core.GraphQLTokenizer(text);
        Assert.Equal(GraphQLTokenKind.StartOfText, t.TokenKind);
        t.Next();
        Assert.Equal(GraphQLTokenKind.IntValue, t.TokenKind);
    ...
    [Theory]
    [InlineData("0.0", 0.0, GraphQLTokenKind.EndOfText)]
    [InlineData("9.1", 9.1, GraphQLTokenKind.EndOfText)]
    [InlineData("12345.0123", 12345.0123, GraphQLTokenKind.EndOfText)]
    [InlineData("9876543.987", 9876543.987, GraphQLTokenKind.EndOfText)]
    [InlineData("-0.0", -0.0, GraphQLTokenKind.EndOfText)]
    [InlineData("-9.1", -9.1, GraphQLTokenKind.EndOfText)]
    [InlineData("-12345.0123", -12345.0123, GraphQLTokenKind.EndOfText)]
    [InlineData("-9876543.987", -9876543.987, GraphQLTokenKind.EndOfText)]
    [InlineData("1e0", 1e0, GraphQLTokenKind.EndOfText)]
    [InlineData("2e123", 2e123, GraphQLTokenKind.EndOfText)]
    [InlineData("1e+2", 1e+2, GraphQLTokenKind.EndOfText)]
    [InlineData("2e+33", 2e+33, GraphQLTokenKind.EndOfText)]
    [InlineData("1e-2", 1e-2, GraphQLTokenKind.EndOfText)]
    [InlineData("2e-33", 2e-33, GraphQLTokenKind.EndOfText)]
    [InlineData("1.1e0", 1.1e0, GraphQLTokenKind.EndOfText)]
    [InlineData("2.2e123", 2.2e123, GraphQLTokenKind.EndOfText)]
    [InlineData("1.1e+2", 1.1e+2, GraphQLTokenKind.EndOfText)]
    [InlineData("2.2e+33", 2.2e+33, GraphQLTokenKind.EndOfText)]
    [InlineData("1.1e-2", 1.1e-2, GraphQLTokenKind.EndOfText)]
    [InlineData("2.2e-33", 2.2e-33, GraphQLTokenKind.EndOfText)]
    [InlineData("0.0 ", 0.0, GraphQLTokenKind.EndOfText)]
    [InlineData("9.1#", 9.1, GraphQLTokenKind.EndOfText)]
    [InlineData("12345.0123\r\n", 12345.0123, GraphQLTokenKind.EndOfText)]
    [InlineData("9876543.987{", 9876543.987, GraphQLTokenKind.LeftCurlyBracket)]
    [InlineData("-0.0﻿\uFEFF", -0.0, GraphQLTokenKind.EndOfText)]
    public void FloatValue(string text, double val, GraphQLTokenKind nextToken)
    {
        var t = new Core.GraphQLTokenizer(text);
        Assert.Equal(GraphQLTokenKind.StartOfText, t.TokenKind);
        t.Next();
        Assert.Equal(GraphQLTokenKind.FloatValue, t.TokenKind);
    ...
    public void SimpleString(string text, string contents)
    {
        ...
        Assert.Equal(GraphQLTokenKind.StartOfText, t.TokenKind);
        t.Next();
        Assert.Equal(GraphQLTokenKind.StringValue, t.TokenKind);

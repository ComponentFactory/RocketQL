...
public class ExtendEnumTypeDefinition
{
    ...
    {
        var documentNode = Document.SchemaDeserialize("extend enum foo { BUZZ }");
    ...
    {
        var documentNode = Document.SchemaDeserialize("extend enum foo @bar");
    ...
    {
        var documentNode = Document.SchemaDeserialize("extend enum foo @bar { BUZZ }");
    ...
    public void ExtendEnumTypeMissingAtLeastOne()
    {
        try
        {
            var documentNode = Document.SchemaDeserialize("extend enum foo 42");
        }
        catch (SyntaxException ex)
        {

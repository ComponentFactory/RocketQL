...
public class DirectiveDefinition
{
    ...
    {
        var t = new Core.GraphQLParser($"directive @foo on {location}");
        var documentNode = t.Parse();
    ...
    {
        var t = new Core.GraphQLParser($"directive @foo on {str}");
        var documentNode = t.Parse();
    ...
    {
        var t = new Core.GraphQLParser("directive @foo (bar: fizz) on ENUM");
        var documentNode = t.Parse();
    ...
    {
        var t = new Core.GraphQLParser("directive @foo (bar: fizz = 3.14) on ENUM");
        var documentNode = t.Parse();
    ...
    [Theory]
    [InlineData("directive foo", GraphQLTokenKind.At, GraphQLTokenKind.Name)]
    [InlineData("directive @42", GraphQLTokenKind.Name, GraphQLTokenKind.IntValue)]
    [InlineData("directive @foo repeatable 42", GraphQLTokenKind.Name, GraphQLTokenKind.IntValue)]
    [InlineData("directive @foo (bar: fizz @hello) on 42", GraphQLTokenKind.Name, GraphQLTokenKind.IntValue)]

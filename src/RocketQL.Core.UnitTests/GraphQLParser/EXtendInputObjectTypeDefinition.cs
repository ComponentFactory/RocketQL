...
public class ExtendInputObjectTypeDefinition
{
    ...
    public void Minimum()
    {
        var t = new Core.GraphQLParser("extend input foo { bar: Integer }");
    ...
    {
        var t = new Core.GraphQLParser("extend input foo @fizz { bar: Integer @buzz }");
        var documentNode = t.Parse();
    ...
    {
        var t = new Core.GraphQLParser("extend input foo 42");
        try
        {
            var documentNode = t.Parse();
        }
